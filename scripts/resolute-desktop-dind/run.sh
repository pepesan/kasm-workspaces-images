...
PORT=6913
CONTAINER_NAME="kasm-resolute-dind-test"
# ============================================
...
echo -e "${YELLOW}==================================================${NC}"
echo -e "${YELLOW}  Lanzando contenedor Kasm Resolute Desktop + DinD ${NC}"
echo -e "${YELLOW}==================================================${NC}"
echo ""
...
if ! docker image inspect "${IMAGE_NAME}:${IMAGE_TAG}" &>/dev/null; then
  ...
  echo -e "${YELLOW}  Ejecuta primero: ./scripts/resolute-desktop-dind/build.sh${NC}"
  exit 1
fi
