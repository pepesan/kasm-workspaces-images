# Imagen Kasm — Entorno de desarrollo Python

**Imagen:** `pepesan/mi-ubuntu-noble-kasm-python`
**Dockerfile:** `dockerfile-kasm-ubuntu-noble-desktop-python`

Imagen de escritorio sobre Ubuntu 24.04 (Kasm) con entorno completo para desarrollo en Python.

## Contenido de la imagen

- **Python 3** (versión de Ubuntu Noble) + `python3-venv`, `python3-pip`, `python3-dev`
- **pipx**, **uv** — gestión de entornos y paquetes Python
- Drivers de acceso a MariaDB: `python3-pymysql` y `python3-mariadb`
- **PyCharm Community** (JetBrains) — última release
...
./scripts/python/stop.sh
./scripts/python/destroy.sh
./scripts/python/push.sh
```
...
Ubuntu 24.04 marca el Python del sistema como "externally managed" (PEP 668): instala paquetes siempre dentro de un `venv` o con `pipx`/`uv`, no con `pip install` directo a nivel de sistema.
